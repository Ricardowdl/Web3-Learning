...
export async function sendTransaction(provider: ethers.BrowserProvider, toAddress: string, amount: string): Promise<TransactionResult> {
  try {
...
export async function estimateGas(provider: ethers.BrowserProvider, toAddress: string, amount: string): Promise<string> {
  try {
    const signer = await provider.getSigner();
    const amountInWei = ethers.parseEther(amount);
