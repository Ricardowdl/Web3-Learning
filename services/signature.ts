...
export const createSignatureService = (provider: ethers.BrowserProvider) => {
  const signMessage = async (message: string): Promise<SignMessage> => {
    try {
  ...
  const verifyMessage = async (message: string, signature: string): Promise<VerifyResult> => {
    try {
  ...
  return { signMessage, verifyMessage, signTypedData, formatMessage, generateRandomMessage };
};
